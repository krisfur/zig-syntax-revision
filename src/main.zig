...
pub fn main() !void {
    ...
    // using an impotred function from a C style library
    ...
    try std.testing.expect(@TypeOf(ptr) == *i32); //the type of a pointer is *type
    ...
    try std.testing.expect(ptrArray[0] == 1);
    ptrArray += 1;
    try std.testing.expect(ptrArray[0] == 2);
    ...
    try std.testing.expect(array2[3] == 4);
    slice[1] += 1;
    try std.testing.expect(array2[3] == 5);
